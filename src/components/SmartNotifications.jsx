...
const SmartNotifications = ({ user, onRecommendationClick }) => {
  ...
  const loadNotifications = async () => {
    ...
    try {
      ...
      if (analysis && !analysis.error) {
        ...
        const urgentRecommendations = analysis.recommendations?.filter(rec => 
        ...
        if (analysis.overallProgress?.recentTrend === 'declining-fast') {
          smartNotifications.push({
            ...
            message: 'Your recent scores have dropped significantly. Time to refocus!',
        ...
        const streakData = analysis.detailedInsights?.find(insight => 
        ...
        const lastQuizDate = analysis.overallProgress?.totalQuizzes > 0 ? 
          new Date() : null // Simplified - would use actual last quiz date
        
        if (lastQuizDate) {
          const daysSinceLastQuiz = Math.floor((new Date() - lastQuizDate) / (1000 * 60 * 60 * 24))
        ...
        const improvements = analysis.detailedInsights?.filter(insight => 
          insight.type === 'improvement' && insight.positive
        ) || []
        ...
        setNotifications(smartNotifications.filter(n => !dismissed.has(n.id)))
      }
    } catch (error) {
  ...
  const getTypeColor = (type) => {
    switch (type) {
      ...
      case 'motivation': return 'text-orange-400'
      case 'celebration': return 'text-green-400'
  ...
  if (notifications.length === 0) {
    return (
      <div className="text-center py-8">
        ...
        <p className="text-gray-400 text-sm">No new recommendations or alerts.</p>
      </div>
    )
  }
              ...
              <div className="flex-1 min-w-0">
                ...
                {notification.action && (
                  <div className="mt-3 p-2 bg-white/5 rounded border border-white/10">
                    ...
                    <div className="text-sm text-white">{notification.action}</div>
                  </div>
                )}

                {notification.category && notification.type === 'recommendation' && (
                  <div className="mt-3 flex gap-2">
                    <button
                      onClick={() => onRecommendationClick?.(notification.category)}
