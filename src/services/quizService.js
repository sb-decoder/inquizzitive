      ...
      if (data && data[0]) {
        try {
          ...
          if (user) {
            ...
            setTimeout(async () => {
              try {
                const analysis = await analyticsService.analyzeWeakness(
                  user.id,
                );
  ...
  async getQuizStats() {
    try {
      const { data, error } = await supabase
        .from("quiz_history")
        .select(
          "category, difficulty, score_percentage, total_questions, correct_answers, created_at",
        );
      ...
      data.forEach((quiz) => {
        ...
        stats.categoryStats[quiz.category].totalScore += parseFloat(
      ...
      Object.keys(stats.categoryStats).forEach((category) => {
        ...
        categoryData.averageScore = (
          categoryData.totalScore / categoryData.count
        ).toFixed(1);
      });
      ...
      data.forEach((quiz) => {
        ...
        stats.difficultyStats[quiz.difficulty].totalScore += parseFloat(
          quiz.score_percentage,
        );
      });
      ...
      Object.keys(stats.difficultyStats).forEach((difficulty) => {
        ...
        difficultyData.averageScore = (
          difficultyData.totalScore / difficultyData.count
        ).toFixed(1);
      });
