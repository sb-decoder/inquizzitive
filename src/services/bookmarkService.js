  ...
  async getBookmarkStats() {
    try {
      ...
      data.forEach((bookmark) => {
        // Category counts
        stats.categoryCounts[bookmark.category] =
          (stats.categoryCounts[bookmark.category] || 0) + 1;

        // Difficulty counts
        stats.difficultyCounts[bookmark.difficulty] =
          (stats.difficultyCounts[bookmark.difficulty] || 0) + 1;
  ...
  async searchBookmarks(searchTerm, limit = 20) {
    try {
      ...
      const { data, error } = await supabase
        ...
        .eq("user_id", user.id)
        .or(
          `question.ilike.%${searchTerm}%, explanation.ilike.%${searchTerm}%, notes.ilike.%${searchTerm}%`,
        )
        .order("created_at", { ascending: false })
        .limit(limit);
