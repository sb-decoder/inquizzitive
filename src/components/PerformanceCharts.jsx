...
const getPeriodValue = (label) =>
  PERIOD_OPTIONS.find((opt) => opt.label === label)?.value;
...
const getChartValue = (label) =>
  CHART_OPTIONS.find((opt) => opt.label === label)?.value;
...
const PerformanceCharts = ({ user }) => {
  ...
  const loadChartData = async () => {
    ...
    try {
      const result = await analyticsService.getPerformanceChartData(
        user.id,
        selectedPeriod,
      );
        ...
        <div className="bg-white/5 backdrop-blur-md rounded-xl p-6 border border-white/20">
          ...
          <div className="w-full overflow-x-auto">
            <svg width={width} height={height} className="w-full h-auto">
              ...
              {validData.map((item, index) => {
                if (
                  index % Math.max(1, Math.floor(validData.length / 5)) ===
                  0
                ) {
                  const x = getX(index);
  ...
  if (
    !chartData ||
    (chartData.overall?.length === 0 &&
      Object.keys(chartData.categoryTrends || {}).length === 0)
  ) {
    return (
      <div className="text-center py-12">
        <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-6 max-w-md mx-auto">
          <p className="text-yellow-400">
            📈 Take more quizzes in the selected time period to see your
            performance trends.
          </p>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-6">
        {(selectedChart === "overall" || selectedChart === "both") &&
          hasOverallData && (
            <LineChart
        ...
        {(selectedChart === "category" || selectedChart === "both") &&
          hasCategoryData && (
            <div className="space-y-6">
