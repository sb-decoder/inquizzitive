...
export const analyticsService = {
  async getPerformanceChartData(userId, days = 30) {
    try {
      console.log(
        "📊 Fetching performance chart data for user:",
        userId,
        "days:",
        days,
      );
      ...
      Object.keys(categoryData).forEach((category) => {
        ...
        categoryTrends[category] = Object.keys(dailyAvg)
          .map((date) => ({
            date,
            averageScore:
              dailyAvg[date].reduce((sum, score) => sum + score, 0) /
              dailyAvg[date].length,
  ...
  async analyzeWeakness(userId) {
    try {
      ...
      const weakAreas = Object.entries(categoryStats)
        .filter(([, stats]) => stats.averageScore < 70)
        .map(([category, stats]) => ({
          category,
      ...
      const strengths = Object.entries(categoryStats)
        .filter(([, stats]) => stats.averageScore >= 80)
        .map(([category, stats]) => ({
          category,
          averageScore: stats.averageScore.toFixed(1),
          totalQuizzes: stats.totalQuizzes,
      ...
      return {
        ...
        overallProgress: {
          ...
          averageScore: (
            quizHistory.reduce(
              (sum, quiz) => sum + parseFloat(quiz.score_percentage),
              0,
            ) / quizHistory.length
          ).toFixed(1),
          recentTrend,
  ...
  calculateRecentTrend(timeBasedStats) {
    ...
    const sortedStats = timeBasedStats.sort(
    ...
    const recentAvg =
      recent.reduce((sum, stat) => sum + stat.score, 0) / recent.length;
    const earlierAvg =
      earlier.reduce((sum, stat) => sum + stat.score, 0) / earlier.length;
  ...
  calculateOverallImprovement(timeBasedStats) {
    ...
    const firstAvg =
      firstHalf.reduce((sum, stat) => sum + stat.score, 0) / firstHalf.length;
    const secondAvg =
      secondHalf.reduce((sum, stat) => sum + stat.score, 0) / secondHalf.length;
  ...
  generateRecommendations(
    ...
    const totalQuizzes = Object.values(categoryStats).reduce(
      (sum, stat) => sum + stat.totalQuizzes,
      0,
    );
    if (totalQuizzes < 20) {
      recommendations.push({
    ...
    if (easyAvg > 80 && mediumAvg < 60) {
      recommendations.push({
        ...
        description:
          "You've mastered Easy questions. Time to challenge yourself with Medium difficulty.",
    ...
    if (recentTrend === "declining" || recentTrend === "declining-fast") {
      recommendations.push({
        ...
        description:
          "Your recent performance has dropped. Let's get back on track.",
        action:
          "Review your weak areas and take easier quizzes to rebuild confidence",
        icon: "🔄",
    ...
    const inconsistentCategories = Object.entries(categoryStats)
      .filter(([, stats]) => this.calculateConsistency(stats) < 0.7)
  ...
  generateDetailedInsights(categoryStats, quizHistory) {
    ...
    const improvedCategories = Object.entries(categoryStats)
      .filter(([, stats]) => stats.improvementRate > 5)
  ...
  analyzeStudyTime(quizHistory) {
    ...
    const totalTime = timeTaken.reduce(
      (sum, quiz) => sum + quiz.timePerQuestion * quiz.totalQuestions,
      0,
    );
    const totalQuestions = timeTaken.reduce(
